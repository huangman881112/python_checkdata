...
for bill_no in bill_nolist:
    ...
    if response_records[0]["status"] in [2]:
        exist = True
        for bill in response_records:
            if (bill["shipmentPlanId"] == bill_no
                    and bill["totalSkuQty"] == qty):
                exist=False
                # result = bill_no+", "+ str(qty)+", "+str(bill["totalSkuQty"])
                # print(result)
                break
            else:
                # result = bill_no+", "+ str(qty)+", "+str(bill["totalSkuQty"])
