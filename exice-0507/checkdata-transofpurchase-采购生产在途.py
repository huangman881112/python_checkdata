...
po_list = ''
with open('purchases_生产在途_po.txt', 'r', encoding='utf-8') as file:
    # content = file.read()  # 一次性读取整个文件内容
    # 或者使用以下方式逐行读取
    for line in file:
        po_list += line
...
po_zip_list = ''
with open('purchases_生产在途_po_zpo.txt', 'r', encoding='utf-8') as file:
    # content = file.read()  # 一次性读取整个文件内容
    # 或者使用以下方式逐行读取
    for line in file:
        po_zip_list += line
...
po_zip_list = json.loads(po_zip_list)

result = ''
with open('checkdata-transofpurchase-采购收货在途.txt', 'w', encoding='utf-8') as filewr:
    for bill_no in po_list:

        qty = int(po_list[bill_no])
        # print(qty)
        param_body = apiList["purchase_detail_para_body"]
        param_body["poId"] = bill_no
        response = requests.post(apiList["purchase_detail_url"], json=param_body, headers=headers)
        # print(response.text)
        time.sleep(1)
        response_data = json.loads(response.text)
        response_records = response_data["data"]
        if 'list' in response_records["poDetailEntityPageInfo"]:
            for zop in response_records["poDetailEntityPageInfo"]["list"]:
                po_zpo_bil = zop["poId"] + zop["cpoId"]
                #下单数-收货数-预约-直发数
                pur_qty = zop["ziquantityQty"] - zop["ireceivedQty"] - zop["reservationQty"] - zop["straightHairQty"]-zop["closeQty"]
                if zop["poId"] + zop["cpoId"] in po_zip_list:
                    inv_qty = po_zip_list[zop["poId"] + zop["cpoId"]]
                    if pur_qty != inv_qty:
                        result = po_zpo_bil + "," + str(pur_qty) + "," + str(inv_qty) + "\n"
                        print(result)
                        filewr.write(result)
                else:
                    result = po_zpo_bil + "," + str(pur_qty) + "\n"
                    print(result)
                    filewr.write(result)
        else:
